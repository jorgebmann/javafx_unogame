...
/**
 * This class represents all game rules. First, card on top of deck in the
 * middle of the table is compared to cards on hand. Next, if player can play a
 * card, he does it. This card is now the card on top of table and withdrawn
 * from deck of cards. If no card can be played: draw a card from deck. We have
 * an exception for action cards (handle draw two, reverse) and wild cards (draw
 * four).
 */
public class RulesHuman {
	 ...
	 */
	public RulesHuman(UnoDeck unoDeck, List<Card> player, Card topCard, String cardType) {
		super();
	...
	public RulesHuman(UnoDeck thisDeck, List<Card> playerCards, Card topCard2) {
		...
		if (isHuman) {
			...
			for (int i = 0; i < player.size(); i++) {
				...
				if (cardType == "afterAction" && (topCard.getColor().equals(player.get(i).getColor()))) {
					System.out.println("Top card: " + topCard);
					System.out.println(
							"Player plays a Number card based on color (on reverse or drawtwo or drawfour card): "
									+ player.get(i));
					topCard = player.get(i);
					System.out.println("New top card: " + topCard);
					// Remove card from player:
	...
	public int drawCard(UnoDeck unoDeck, List<Card> player, Card topCard) {
		// if player can not play a card: take one from the deck:
		System.out.println("Top card: " + topCard);
		System.out.println("Player's deck size: " + player.size());
		Card toRemove = unoDeck.getCards().remove(0);
		player.add(toRemove);

		player.add(unoDeck.getCards().remove(0));
		System.out
				.println("Player can't play card and takes one from deck. Player has now " + player.size() + " cards");
		return player.size();
	}
...
//		System.out.println("Number of Cards on hand: " + player.size());
	...
	public int handleActionDraw(UnoDeck unoDeck, List<Card> player, Card topCard) {
